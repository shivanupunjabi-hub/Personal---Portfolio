...
public class FastMediaServer {
    ...
    public int Start() {
        ...
        _running = true;
        Console.WriteLine("HTTP Server running at http://localhost:" + _port + "/");
    ...
    private void ProcessRequest(HttpListenerContext context) {
        ...
        try {
            ...
            // Client closed stream (normal during video seek or navigation)
        } finally {
            try { res.Close(); } catch {}
        }
    }
...
$server = New-Object FastMediaServer($Root, $Port)
